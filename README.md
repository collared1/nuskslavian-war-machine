...
3. put it into a folder in Documents
4. add to the folder an image called "NukeBotImage.png" (capitalization matters)
5. in the file, edit all of the config options, and add a bot token from the [discord developer portal](https://discord.com/developers/applications) into the tokens section, following instructions in the code.
6. go to Command Prompt or Terminal.
7. run `cd "C:Users/YOUR WINDOWS NAME/Documents/YOUR FOLDER NAME"` if you are on windows, and `cd "~/Documents/YOUR FOLDER NAME"` if you are on mac.
8. run `pip install discord.py`
9. run `python bot.py` OR, if you have changed the name of `bot.py`, do `python YOUR FILE NAME`
10. add the bot to a server using the invite link the bot generates
11. run `!nuke` in the server.
12. watch the nukebot do its thing

**this nukebot is kinda shit, ive made a better one but dont wanna leak it. anyhow, have fun!**

**To run it again after closing the terminal/command prompt, redo step 6, and 8-11.**
